...
const url = "header-sales/wasfaty-by-month";
...
export const loader = async ({ request }) => {
  try {
    ...
    throw new Error("Failed to fetch Wasfaty sales by month");
  } catch (error) {
    console.error("Error fetching Wasfaty sales by month:", error);
    ...
    return {
      ...
      availableYears: [],
      selectedYear: null,
      error: error.response?.data?.message || error.message || "Failed to fetch Wasfaty sales by month",
    };
  }
};
...
const Wasfaty = () => {
  ...
  const handleYearChange = (year) => {
    ...
    navigate(`/wasfaty${params.toString() ? `?${params.toString()}` : ''}`);
  };
  ...
  const chartData = sortedSales.map((item, index) => ({
    ...
    wasfaty: Math.round(item.Wasfaty || 0),
    returnWasfaty: Math.round(item.ReturnWasfaty || 0),
    year: item.Year,
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-base-200 p-4 rounded-lg shadow-lg border border-base-300">
          <p className="font-semibold text-base-content">{data.month}</p>
          <p className="text-success">
            <FaDollarSign className="inline mr-1" />
            Total: {formatCurrency(data.amount)}
          </p>
          <p className="text-sm mt-1">
            Wasfaty: {formatCurrency(data.wasfaty)}
          </p>
          <p className="text-sm">
            Return Wasfaty: {formatCurrency(data.returnWasfaty)}
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className='text-6xl font-bold text-blue-500'>Wasfaty Sales by Month</h1>
          <p className="text-lg text-base-content/70 mt-2">
            {selectedYear
              ? `Wasfaty sales data for ${selectedYear}`
              : 'All Wasfaty sales data grouped by month'}
          </p>
        </div>
      ...
      {salesByMonth.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-desc">Months with Wasfaty data</div>
          </div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-desc">Wasfaty + Return Wasfaty</div>
          </div>
        ...
        <div className="card bg-base-100 shadow-lg mb-6">
          <div className="card-body">
            <h2 className="card-title text-2xl mb-4">
              ...
              Wasfaty Sales by Month - Column Chart
            </h2>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Legend
                    ...
                    formatter={() => <span className="text-base-content">Total Amount</span>}
                  />
                  <Bar
                    dataKey="amount"
                    radius={[8, 8, 0, 0]}
      ...
      {sortedSales && sortedSales.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="table table-zebra w-full">
              ...
              {sortedSales.map((item, index) => (
                <tr key={`${item.Year}-${item.Month}-${index}`} className="hover">
                  ...
                  <td className="font-semibold">
                    {formatCurrency(Math.round(item.Wasfaty || 0))}
                  </td>
                  <td className="font-semibold">
                    {formatCurrency(Math.round(item.ReturnWasfaty || 0))}
                  </td>
                  <td className="font-bold text-lg">
                    <div className="flex items-center gap-1">
                      <FaDollarSign className="text-success" />
                      <span className="text-success">
                        {formatCurrency(Math.round(item.Total || 0))}
                      </span>
                    </div>
                  </td>
                </tr>
            ...
            <tfoot>
              <tr>
                <th colSpan={2} className="text-right">Grand Totals:</th>
                <th className="text-success text-lg">
                  {formatCurrency(Math.round(totals.Wasfaty || 0))}
                </th>
                <th className="text-success text-lg">
                  {formatCurrency(Math.round(totals.ReturnWasfaty || 0))}
                </th>
                <th className="text-success text-lg">
        ...
        <div className="text-center py-12">
          <p className="text-2xl text-base-content/70 mb-4">
            {selectedYearState
              ? `No Wasfaty sales data found for year ${selectedYearState}`
              : 'No Wasfaty sales data found'}
          </p>
          {selectedYearState && (
            <button
