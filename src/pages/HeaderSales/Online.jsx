...
const url = 'header-sales/online-by-month';
...
export const loader = async ({ request }) => {
  try {
    ...
    throw new Error('Failed to fetch Online sales by month');
  } catch (error) {
    console.error('Error fetching Online sales by month:', error);
    ...
    return {
      ...
      error:
        ...
        error.message ||
        'Failed to fetch Online sales by month',
    };
  }
};

const Online = () => {
  const { salesByMonth, grandTotals, availableYears, selectedYear, error } =
    useLoaderData();
  ...
  const handleYearChange = (year) => {
    ...
    navigate(`/online${params.toString() ? `?${params.toString()}` : ''}`);
  };
  ...
  const chartData = sortedSales.map((item, index) => ({
    ...
    online: Math.round(item.Online || 0),
    returnOnline: Math.round(item.ReturnOnline || 0),
    year: item.Year,
      ...
      return (
        <div className="bg-base-200 p-4 rounded-lg shadow-lg border border-base-300">
          ...
          <p className="text-success">
            ...
            Total Online: {formatCurrency(data.amount)}
          </p>
          <p className="text-sm mt-1">
            Online: {formatCurrency(data.online)}
          </p>
          <p className="text-sm">
            Return Online: {formatCurrency(data.returnOnline)}
          </p>
        </div>
      );
    }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-lg text-base-content/70 mt-2">
            {selectedYear
              ? `Online (Online + Return Online) sales data for ${selectedYear}`
              : 'All Online (Online + Return Online) sales data grouped by month'}
          </p>
        </div>
      ...
      {salesByMonth.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-desc">Months with Online sales data</div>
          </div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-title">Grand Total Online</div>
            <div className="stat-value text-success">
              {formatCurrency(grandTotal)}
            </div>
            <div className="stat-desc">Online + Return Online</div>
          </div>
        ...
        <div className="card bg-base-100 shadow-lg mb-6">
          <div className="card-body">
            <h2 className="card-title text-2xl mb-4">
              ...
              Online Sales by Month - Column Chart
            </h2>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Legend
                    ...
                    formatter={() => (
                      <span className="text-base-content">Total Online</span>
                    )}
                  />
                  <Bar
                    dataKey="amount"
      ...
      {sortedSales && sortedSales.length > 0 ? (
              ...
              {sortedSales.map((item, index) => (
                ...
                >
                  ...
                  <td className="font-semibold">
                    {formatCurrency(Math.round(item.Online || 0))}
                  </td>
                  <td className="font-semibold">
                    {formatCurrency(Math.round(item.ReturnOnline || 0))}
                  </td>
            ...
            <tfoot>
              <tr>
                ...
                <th className="text-success text-lg">
                  {formatCurrency(Math.round(totals.Online || 0))}
                </th>
                <th className="text-success text-lg">
                  {formatCurrency(Math.round(totals.ReturnOnline || 0))}
                </th>
                <th className="text-success text-lg">
                  {formatCurrency(grandTotal)}
                </th>
        ...
        <div className="text-center py-12">
          <p className="text-2xl text-base-content/70 mb-4">
            {selectedYearState
              ? `No Online sales data found for year ${selectedYearState}`
              : 'No Online sales data found'}
          </p>
          {selectedYearState && (
