...
const url = "header-sales/insurance-by-month";
  ...
  try {
    ...
    if (response.data.success) {
      return {
        ...
        insuranceInvoiceTypes: response.data.insuranceInvoiceTypes || [],
      };
    }
    throw new Error("Failed to fetch insurance sales by month");
  } catch (error) {
    console.error("Error fetching insurance sales by month:", error);
    ...
    return {
      ...
      error: error.response?.data?.message || error.message || "Failed to fetch insurance sales by month",
    };
  }
};

const Insurance = () => {
  const { salesByMonth, grandTotals, availableYears, selectedYear, insuranceInvoiceTypes, error } = useLoaderData();
  ...
  const handleYearChange = (year) => {
    ...
    navigate(`/insurance${params.toString() ? `?${params.toString()}` : ''}`);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className='text-6xl font-bold text-blue-500'>Insurance Sales by Month</h1>
          <p className="text-lg text-base-content/70 mt-2">
            {selectedYear 
              ? `Insurance sales data for ${selectedYear}`
              : 'All insurance sales data grouped by month'
            }
          </p>
        </div>
      ...
      {salesByMonth.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-desc">Sum of all insurance invoice types</div>
          </div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-desc">Number of insurance invoices</div>
          </div>
        </div>
      )}
        ...
        <div className="card bg-base-100 shadow-lg mb-6">
          <div className="card-body">
            <h2 className="card-title text-2xl mb-4">
              ...
              Insurance Sales by Month - Column Chart
            </h2>
      ...
      {sortedSales && sortedSales.length > 0 ? (
          ...
          <table className="table table-zebra w-full">
            ...
            <tbody>
              {sortedSales.map((item, index) => (
                <tr key={`${item.Year}-${item.Month}-${index}`} className="hover">
                  ...
                  {insuranceInvoiceTypes.map(type => (
                    <td key={type} className="font-semibold">
                      {formatCurrency(Math.round(item[type] || 0))}
                    </td>
                  ))}
            ...
            <tfoot>
              <tr>
                ...
                {insuranceInvoiceTypes.map(type => (
                  <th key={type} className="font-semibold">
                    {formatCurrency(Math.round(totals[type] || 0))}
                  </th>
                ))}
        ...
        <div className="text-center py-12">
          <p className="text-2xl text-base-content/70 mb-4">
            {selectedYearState 
              ? `No insurance sales data found for year ${selectedYearState}`
              : 'No insurance sales data found'
            }
          </p>
          {selectedYearState && (
            <button
