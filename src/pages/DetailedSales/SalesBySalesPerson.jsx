...
import { FaUser, FaArrowLeft, FaDollarSign, FaShoppingCart, FaBox, FaChartBar, FaFilter, FaCalendarAlt } from 'react-icons/fa';
...
const SalesBySalesPerson = () => {
  ...
  const fetchPharmacies = async () => {
    try {
      const res = await customFetch.get('pharmacies');
      if (res.data?.success) {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await customFetch.get('detailed-sales/stats/sales-by-name', { params });
      if (res.data?.success) {
  ...
  const handleBranchCodeChange = (e) => {
    ...
    const params = {};
    if (newBranchCode) params.branchCode = newBranchCode;
  ...
  const selectedPharmacy = pharmacies.find(p => p.branchCode === parseInt(branchCode || 0));
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-base-content/70">
          Statistics showing sales data for each sales person
          {selectedPharmacy && (
            <span className="ml-2">
              ...
              <div className="card-body">
                <div className="flex items-center gap-2 md:gap-3">
                  <div className="p-2 md:p-3 bg-primary/10 rounded-lg flex-shrink-0">
                    <FaUser className="text-xl md:text-2xl text-primary" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <h3 className="text-xs md:text-sm text-base-content/70">Total Sales Persons</h3>
                    <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold">
                      {summary?.totalSalesPersons || 0}
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    {stats.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan="6" className="text-center py-8">
                          <p className="text-base-content/70">No sales data available for sales persons.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
                      ...
                      <th>
                        <span className="text-sm sm:text-base md:text-lg font-semibold text-primary">
                          {(() => {
                            ...
                            const totalTransactions = stats.reduce((sum, s) => sum + s.totalTransactions, 0);
