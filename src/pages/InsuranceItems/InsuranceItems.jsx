...
export const loader = async ({ request }) => {
  try {
    ...
    if (params.get('minSapCode')) queryParams.minSapCode = params.get('minSapCode');
    if (params.get('maxSapCode')) queryParams.maxSapCode = params.get('maxSapCode');
    if (params.get('sortBySapCode')) queryParams.sortBySapCode = params.get('sortBySapCode');
    ...
    return {
      ...
      page: 1,
      pages: 0,
      error: error.response?.data?.message || error.message || "Failed to fetch insurance items",
    };
  }
};
...
const InsuranceItems = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    maxSapCode: searchParams.get('maxSapCode') || '',
  });
  ...
  // Toggle sort by SAP Code (updates URL to trigger backend sorting)
  const handleSortBySapCode = () => {
    const params = new URLSearchParams(searchParams);
    const currentSort = params.get('sortBySapCode');
    
    // Cycle through: no sort -> desc -> asc -> no sort
    if (!currentSort || currentSort === '') {
      params.set('sortBySapCode', 'desc');
    } else if (currentSort === 'desc') {
      params.set('sortBySapCode', 'asc');
    } else {
      params.delete('sortBySapCode');
    }
    
    // Reset to page 1 when changing sort
    params.set('page', '1');
    
    navigate(`/insurance-items?${params.toString()}`);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className='text-6xl font-bold text-blue-500'>Insurance Items</h1>
        ...
        <div className="card bg-base-100 shadow-lg mb-6">
          <div className="card-body">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text">Min SAP Code</span>
                </label>
                <input
                  type="number"
                  placeholder="0"
                  className="input input-bordered"
                  value={filters.minSapCode}
                  onChange={(e) => setFilters({ ...filters, minSapCode: e.target.value })}
                />
              </div>
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text">Max SAP Code</span>
                </label>
                <input
                  type="number"
                  placeholder="0"
                  className="input input-bordered"
                  value={filters.maxSapCode}
                  onChange={(e) => setFilters({ ...filters, maxSapCode: e.target.value })}
                />
              </div>
            </div>
      ...
      {items && items.length > 0 ? (
            ...
            <thead>
              <tr>
                <th 
                  ...
                  onClick={handleSortBySapCode}
                  title="Click to sort by SAP Code"
                >
                ...
                <tr key={item._id} className="hover">
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-semibold">{item.SAP_Code}</span>
                    </div>
                  </td>
                  <td>
                    {item.Category ? (
        ...
        <div className="text-center py-12">
          <p className="text-2xl text-base-content/70 mb-4">
            {hasActiveFilters ? 'No items found matching your filters' : 'No insurance items found'}
          </p>
          {hasActiveFilters && (
            <button
              className="btn btn-outline"
